...
// Optional: Chapter-level and Book-level reference edges from Neon
LOAD CSV WITH HEADERS FROM 'file:///csv/chapter_references_edges.csv' AS row
MATCH (a:Chapter {id: row[':START_ID']}), (b:Chapter {id: row[':END_ID']})
MERGE (a)-[r:REFERENCES]->(b)
  SET r.score = toFloat(row.score), r.method = row.method;

LOAD CSV WITH HEADERS FROM 'file:///csv/book_references_edges.csv' AS row
MATCH (a:Book {id: row[':START_ID']}), (b:Book {id: row[':END_ID']})
MERGE (a)-[r:REFERENCES]->(b)
  SET r.score = toFloat(row.score), r.method = row.method;
